...
export default class LevelExit extends Entity{
    ...
    tick(game,deltaTime){
        ...
        if (Math.random()< 0.1){
            ...
            game.level.addParticle(new Particle(game.level,(this.position.x-0.5)+Math.random(),this.position.y,(this.position.z-0.5)+Math.random(),9,{x:0,y:1,z:0},Math.max(0.01,Math.random()/20),[c[0],c[1],c[2],1.0],0.09));
        }
    }
}